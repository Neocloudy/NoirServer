	...
	if(len)
		for(var/obj/item/held_item in held_items)
			if(!holding.len)
				...
				holding += ", \a [held_item]"
	return "You can also see [src] on the photo[health < (max_health * 0.75) ? ", looking a bit hurt":""][holding.len ? ". [holding.Join("")].":"."]"
...
/mob/living/proc/updatehealth()
	if(HAS_TRAIT(src, TRAIT_GODMODE))
		return
	set_health(max_health - get_oxy_loss() - get_tox_loss() - get_fire_loss() - get_brute_loss())
	update_stat()
	med_hud_set_health()
	med_hud_set_status()
	update_health_hud()
...
/mob/living/Move(atom/newloc, direct, glide_size_override)
	...
	if(!buckled && !moving_diagonally && loc != old_loc)
		...
		var/health_check = body_position == LYING_DOWN && prob(get_brute_loss() * 200 / max_health)
...
/mob/living/proc/calculate_fitness()
	...
	return ceil(damage * (ceil(athletics_level / 2)) * max_health)
