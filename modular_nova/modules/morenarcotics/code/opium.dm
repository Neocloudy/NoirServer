...
/datum/reagent/drug/opium/heroin/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	if(SPT_PROB(2.5, seconds_per_tick))
		to_chat(M, span_notice("[high_message]"))
	var/need_mob_update = M.adjust_brute_loss(-0.4 * REM * seconds_per_tick, , updating_health = FALSE) //more powerful as a painkiller, possibly actually useful to medical now
	need_mob_update += M.adjust_fire_loss(-0.4 * REM * seconds_per_tick, , updating_health = FALSE)
	if(need_mob_update)
...
/datum/reagent/drug/opium/blacktar/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	if(M.adjust_tox_loss(0.5 * REM * seconds_per_tick, updating_health = FALSE))
		return UPDATE_MOB_HEALTH
