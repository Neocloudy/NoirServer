...
/datum/reagent/medicine/c2/hercuri/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	var/need_mob_update
	// NOVA EDIT CHANGE BEGIN -- Adds check for owner_flags; indented the get_fire_loss check and everything under it, so synths can get cooled down
...
/datum/reagent/medicine/c2/synthflesh/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message = TRUE, touch_protection = 0)
	...
	if (carbies.get_fire_loss() > UNHUSK_DAMAGE_THRESHOLD * 2.5)
		if (show_message)
		...
		return
	else if (carbies.get_fire_loss() > UNHUSK_DAMAGE_THRESHOLD)
		if (show_message)
