...
/obj/item/healthanalyzer/interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	switch (scanmode)
		if (SCANMODE_HEALTH)
			...
			if((M.health / M.max_health) > CLEAN_BILL_OF_HEALTH_RATIO)
...
/proc/healthscan(mob/user, mob/living/target, mode = SCANNER_VERBOSE, advanced = FALSE, tochat = TRUE)
	...
	var/mob_status = (!target.appears_alive() ? span_alert("<b>Deceased</b>") : "<b>[round(target.health / target.max_health, 0.01) * 100]% healthy</b>")
