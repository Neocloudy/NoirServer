	...
	if(SPT_PROB(2.5, seconds_per_tick) && iscarbon(drinker))
		...
		if(eyes && IS_ORGANIC_ORGAN(eyes)) // doesn't affect robotic eyes
			if(drinker.is_blind())
				...
				if(eyes.apply_organ_damage(eyes.max_health))
					. = UPDATE_MOB_HEALTH
				drinker.emote("scream")
...
/datum/reagent/consumable/ethanol/brave_bull/on_mob_metabolize(mob/living/drinker)
	...
	drinker.max_health += 10 //Brave Bull makes you sturdier, and thus capable of withstanding a tiny bit more punishment.
	drinker.health += 10
...
/datum/reagent/consumable/ethanol/brave_bull/on_mob_end_metabolize(mob/living/drinker)
	...
	drinker.max_health -= 10
	drinker.health = min(drinker.health - 10, drinker.max_health) //This can indeed crit you if you're alive solely based on alchol ingestion
